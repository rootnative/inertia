...
| `focusVisible` | Focus arrived from the keyboard (W3C `:focus-visible`). On native — where focus always arrives via D-pad / hardware keyboard / screen reader — behaves identically to `focused`. | `onFocus` + module-level input-modality tracker (web `keydown` vs `pointerdown` / `mousedown` / `touchstart`). The tracker's `document` listeners attach when the first component that declares `focusVisible` mounts, not at import, so the package stays `sideEffects: false`. |
