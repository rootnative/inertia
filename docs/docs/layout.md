...
- **Rect-only.** Only the four FLIP transforms (`translateX/Y`, `scaleX/Y`) are animated. Other style props — `borderRadius`, `backgroundColor`, `padding` — snap. Style-prop interpolation is not scheduled.
