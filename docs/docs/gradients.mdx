---
sidebar_position: 9
description: "@rootnative/inertia-gradients adds an animatable linear gradient on expo-linear-gradient — an optional sibling package."
---
...
`@rootnative/inertia-gradients` adds an animatable linear gradient built on `expo-linear-gradient`. It is an **optional** sibling package — install it only when you need to animate gradient stops. The core library has no required `expo-linear-gradient` dependency.
