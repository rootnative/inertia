...
Internally this routes through `buildReleaseAnimation` exported from `@onlynative/inertia` — a worklet-safe builder that covers spring / timing / decay / no-animation single-step transitions. Reach for it directly if you're writing a custom `Gesture.Pan().onEnd(...)` worklet outside `useDrag`.
...
All three hooks layer on `react-native-gesture-handler`, which surfaces touch and mouse events but **not keyboard**. For controls that need keyboard a11y in addition to drag — a Slider with arrow-key increment / decrement, a scrollbar with `PageUp` / `PageDown`, a range input — you'll want to keep `PanResponder` for the pointer track and add `onKeyDown` handling alongside it. That doesn't preclude using Inertia's value-layer hooks (`useAnimation`, `useSpring`) to drive the release animation off the JS-thread state the keyboard handlers maintain; the boundary is just that this adapter package can't deliver the keyboard half.

When you outgrow that boundary, the typical migration ladder is:

1. `Motion.View` + `gesture` prop — pure state-overlay UX (hover, focus, press).
2. `useDrag` / `useSwipe` / `usePan` from this package — pointer-driven continuous gestures, no keyboard needs.
3. `PanResponder` + `useAnimation` — pointer + keyboard, full a11y surface, design-your-own gesture lifecycle.

## Try it

<RunnableExample screen="drag" height={620} />
