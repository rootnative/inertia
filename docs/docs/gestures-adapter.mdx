...
All three hooks layer on `react-native-gesture-handler`, which surfaces touch and mouse events but **not keyboard**. For controls that need keyboard a11y in addition to drag — a Slider with arrow-key increment / decrement, a scrollbar with `PageUp` / `PageDown`, a range input — reach for [`useTouchDrag`](./api/hooks#usetouchdragoptions) from `@onlynative/inertia/touch` instead. It's a PanResponder-backed counterpart to `useDrag` that lives in core (no `react-native-gesture-handler` peer required) and composes cleanly with `onKeyDown` on the wrapping `Pressable`.

The full ladder:

1. `Motion.View` + `gesture` prop — pure state-overlay UX (hover, focus, press).
2. `useTouchDrag` from `@onlynative/inertia/touch` — pointer + keyboard drag, no extra peer dep.
